...
- **Detalhe:** `EntityIdentityHeader` + `EntityTabs` (sincroniza `?tab=`). A ação primária (Editar / Vincular / Adicionar) vive no `actions` do header e **muda conforme `sp.tab`** — o Server Component decide qual injetar. **Nunca** pôr essa ação no corpo da tab nem fixa em todas as abas. **Exceção — piloto client-side (`tools/[id]`, PR #259 / ADR-0024):** usa `ToolDetailTabs` (shell client) — troca de tab é 100% cliente (`history.replaceState`, sem tocar o servidor); a ação do header é reativa via `useActiveTab` (não `sp.tab`); tabs eager de `detail` + lazy via `"use server"` action. As outras 8 páginas seguem o `EntityTabs` server-nav até serem migradas (issue de generalização aberta).
