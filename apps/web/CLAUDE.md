...
- **Slug auto-gerado em modo `create`:** `<Input disabled />` com valor derivado do label/nome via `slugifyLabel()` em `dashboard/attributes/schema.ts`. Em `edit` fica editável com hint "alterar pode quebrar URLs/referências".
...
- **Specs dinâmicas:** form busca `definitionsByCategory[primaryCategoryId]` (server-side via `tools/_components/attribute-helpers.ts`). Inputs renderizados por `inputType` em `tools/_components/dynamic-specs-editor.tsx`. Trocar categoria com specs preenchidas → `updateTool` devolve `actionResult.warning = "orphan_attributes"`; form pede confirmação antes de deletar.
- **Painel de atributos por categoria:** `categories/[id]/edit` mostra atributos próprios + herdados de ancestrais + globais com badges de origem (`categories/_components/category-attributes-panel.tsx`). Botão "Novo atributo" linka `/dashboard/attributes/new?categoryId=<id>`.
