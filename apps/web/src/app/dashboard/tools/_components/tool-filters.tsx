	...
	return (
		<FiltersBar hasActive={hasActive} onClear={clearAll}>
			...
			<div className="flex flex-col gap-1 md:w-40">
				<label className="text-muted-foreground text-xs" htmlFor="tool-mode">
					Estoque
				</label>
				<Select
				...
				>
					<SelectTrigger id="tool-mode">
						<SelectValue>
							{(v: string) =>
								v === ALL ? "Todas" : (MODE_LABELS[v] ?? "Todas")
							}
						</SelectValue>
					</SelectTrigger>
					<SelectContent>
						<SelectGroup>
							...
							<SelectItem value="repor">Repor agora</SelectItem>
							<SelectItem value="esgotado">Esgotadas</SelectItem>
						</SelectGroup>
					</SelectContent>
				</Select>
			</div>
