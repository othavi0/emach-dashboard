// @ts-nocheck
//
// PRESERVAÇÃO — matriz role-based original, snapshotada em 2026-05-27.
// Contexto completo no ADR: docs/adr/0012-disable-role-based-gates.md (criado neste mesmo branch).
// Este arquivo NÃO é importado em runtime. Para reativar os gates, copiar este conteúdo
// de volta pra `permissions.ts` (que terá sido convertido em no-op pelas tasks subsequentes
// deste plano), restaurar `branch-scope.ts` e a checagem de `ROLE_WEIGHT` em
// `session.ts:requireRole`. Auditar `user_branch` antes de religar.
