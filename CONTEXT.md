...
- **"Sem Categoria"** — o seed cria uma categoria-raiz catch-all "Sem Categoria" para Tools sem classificação (o `CLAUDE.md` a chamava erroneamente de "Geral"). Resolvido: toda Tool deve ter uma Category **real** — "Sem Categoria" é um smell a remover, tornando a associação Tool–Category obrigatória. Dependência em aberto: realojar os attribute definitions hoje parqueados nessa raiz antes de removê-la.
