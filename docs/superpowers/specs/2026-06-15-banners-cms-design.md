# Banners da home — CMS no dashboard
...
> Data: 2026-06-15.
>
> **Nota de nomenclatura:** o issue #173 usa "hero/hero_slide". Decidiu-se padronizar **"banner"** em tudo (UI, rota, tabela, código) — termo mais claro pra staff leiga. O componente do storefront continua se chamando `HeroCarousel` (nome real existente no ecommerce); o issue futuro do ecommerce deve consumir a tabela `banner`.

## 1. Objetivo

Tornar o hero/carrossel da home do storefront — hoje **hardcoded** (`apps/web/src/components/hero-carousel.tsx` no `emach-ecommerce`: array `HERO_SLIDES` com 2 slides fixos `{ bg, product, alt }`, CTA global fixo `Ver Catálogo → /catalog`, sem título/SEO) — **gerenciável pelo dashboard**: staff cria/edita/ordena/publica banners com imagem de fundo, ferramenta central, título/subtítulo e botão (texto + link).
...
Consequência: este issue **cria dados** (`title`, `subtitle`, `ctaLabel`, `ctaHref` por banner) que **o storefront ainda não sabe exibir**. O preview ao vivo do dashboard mostra a **composição pretendida**; a posição final do texto/CTA é decisão do **refactor do storefront** (issue futuro). O preview rotula isso como "texto indicativo".

## 3. Decisões (brainstorming)

| Decisão | Escolha |
|---|---|
| Nomenclatura | **"banner"** em tudo (UI, rota, tabela, código) |
| Rota | `/dashboard/site/banners` (dentro do grupo `/site/`, junto com Notificações) |
| Nav | Reativar o item "Banners" (`nav-config.ts`) — só remover `disabled` (href já aponta pra cá) |
...
Novo arquivo `packages/db/src/schema/banner.ts` + `export * from "./banner"` no barrel `schema/index.ts`. Convenções de `packages/db/CLAUDE.md` (ID `text` PK por `crypto.randomUUID()` no caller; `timestamptz`). `bun db:sync` após criar.

| coluna | tipo | nota |
|---|---|---|
| `id` | `text` PK | `crypto.randomUUID()` no caller |
| `backgroundImageUrl` | `text NOT NULL` | bucket `banner-images`, URL pública absoluta |
...
- `BANNER_IMAGES_BUCKET = "banner-images"` em `apps/web/src/lib/supabase-server.ts`. Bucket **público**, criado no Supabase (espelha `tool-images`).
...
- `deleteBanner` remove as imagens do bucket junto com a linha.
...
apps/web/src/app/dashboard/site/banners/
  ...
  _components/
    banner-card.tsx             # media-card (layout aprovado: imagem limpa + corpo)
    banner-list.tsx             # grid 3-col + dnd-kit; seções "No ar" / "Rascunhos"
    banner-form.tsx             # 2 colunas (form + preview), compartilhado new/edit
    banner-live-preview.tsx     # toggle desktop/mobile, espelha o hero do storefront
    image-upload-tile.tsx       # 4×, com guidelines visíveis + preview
    delete-banner-dialog.tsx    # AlertDialog destrutivo
    banner-schema.ts            # zod (fonte única form + action)
```

- **Nav** (`apps/web/src/app/dashboard/_components/nav-config.ts`): o item "Banners" já existe com `href: "/dashboard/site/banners"` e `disabled: true` — **só remover o `disabled`**. Mantém label "Banners" e ícone `ImageIcon`.
- **Listagem:** grid de 3 colunas. Duas seções — **"No ar — ordem do carrossel" (N/6 ativos)** com cards arrastáveis (dnd-kit), e **"Rascunhos / despublicados"**. Card tracejado "+ Criar novo banner" no fim. Sem scroll infinito (poucos banners). Empty state via `<Empty>`.
...
- **Form:** duas colunas (`1.15fr .85fr`). Esquerda em seções (**Imagens** 2×2, **Conteúdo**, **Botão**, **Publicação**). Direita: `banner-live-preview` sticky com segmented control 🖥 Desktop / 📱 Mobile. Campos via `<LabeledField>` + `<FieldError>` + `useFormErrors`. Botão Salvar.
- **dnd-kit:** `id` estável no `<DndContext>` (ex: `id="banner-sortable"`) — gotcha de hidratação (DESIGN.md / `categories-tree.tsx`).

## 7. Validação (zod) e guidelines de imagem

`banner-schema.ts` (fonte única form + action):
...
- **Máx 6 banners `isActive = true`** — validado no **server action** (conta no banco; não expressável em zod puro). Ativar um 7º (no toggle da lista ou no publish do form) é bloqueado com mensagem clara. Motivo: autoplay 9s × 6 ≈ ciclo de 54s.
...
`actions.ts`, todas com `"use server"` + `await requireCapability("site.update_banners")` no início + retorno `ActionResult<T>` + `revalidatePath("/dashboard/site/banners")` (e `revalidateTag("site-banners")`). Erros de banco via `getPgError` (não `e.message.includes`). Auditoria: `logUserActivity` com `actorUserId: session.user.id`.

- `createBanner(input)` — nasce `isActive=false`, `sortOrder` = (max atual)+1.
- `updateBanner(id, input)`.
- `deleteBanner(id)` — remove imagens do bucket + linha (`AlertDialog` no client).
- `reorderBanners(orderedIds: string[])` — escreve `sortOrder` em lote.
- `toggleBannerActive(id, active)` — guard dos 6 ativos antes de ativar.
- `uploadBannerImage(formData)` / `deleteBannerImage(url)` — espelham `image-actions.ts`.

Fetchers (Server Component): `fetchBanners()` ordenado por `sortOrder` (separa ativos/rascunhos no render); `fetchBanner(id)` para o edit.

## 9. Decisões menores e fora de escopo

Decisões tomadas:
- `altText` pré-preenche com `title` (editável) — reduz fricção mantendo a11y.
- Banner novo nasce rascunho (`isActive=false`), `sortOrder` = próximo disponível.
...
- `getActiveBanners()` (query SSR `WHERE isActive ORDER BY sortOrder`).
- Refactor do `HeroCarousel` para receber `banners: Banner[]` via props e renderizar `title`/`subtitle`/CTA por banner (SSR p/ SEO) — **decide a posição final do texto**.
...
**Evolução futura (não agora):** agendamento por banner (`startsAt`/`endsAt` + cron de ativação) para banners sazonais (Black Friday etc.). Exige 2 colunas + job cron + lógica de "ativo efetivo" no consumo. Descartado por YAGNI.

## 10. Verificação

- `bun check-types` + `bun check` (ultracite) verdes.
- `bun db:sync` aplica o schema; conferir a tabela `banner` no banco.
- Smoke visual (`bun dev:web`): criar banner com upload, reordenar (drag), publicar/despublicar, validar bloqueio do 7º ativo, conferir preview desktop/mobile, excluir (confirma remoção das imagens do bucket).

## 11. Checklist (espelha o issue #173)

- [ ] Schema `banner` + barrel + `bun db:sync`
- [ ] Bucket `banner-images` + `BANNER_IMAGES_BUCKET` + upload/delete server-side
- [ ] Nav: reativar item "Banners" → `/dashboard/site/banners` (remover `disabled`)
