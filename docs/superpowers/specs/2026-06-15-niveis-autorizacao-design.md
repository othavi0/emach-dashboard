...
- **Estoque** (sem scaffolding): construir o mesmo filtro nas listagens (sem `includeUnassigned` — estoque sempre tem filial) + validar `branchId ∈ scope` nas mutações.
...
Status gate (`pending`/`suspended`), self-action guard, last-super-admin guard — preservados dentro/junto dos gates reativados.

## 7. Migração de dados (ordem importa)

1. `manager → admin` (UPDATE).
2. **Povoar `user_branch`** para todos os admin/user ativos ANTES de religar — fail-closed deixa cego quem não tiver vínculo.
...
## Não decidido (levar pro grill / plano)

- Numeração e título exatos do novo ADR.
