...
| **P0-1** · reembolso (ADR-0025) | ✅ núcleo | `refundOrder()` agora grava `refund_request` (resolve solicitação ativa do cliente ou cria uma já resolvida) — **fecha a divergência de 2 fontes de verdade**. *Follow-up:* actions `approveRefund/rejectRefund/reviewRefund` + UI acionável + `reasonCategory` no dialog (hoje default `outro`). |
| **P0-3** · exceção de separação | ⏳ pendente | Precisa de mudança de fluxo (reportMissing não deve travar a sessão inteira; permitir bipar o resto e finalizar com pendência) + smoke no browser. Recomendação: `reportMissing` marca só o item `notFound` e mantém a sessão `in_progress`; `exception` só na finalização com itens não resolvidos. Não implementado aqui por exigir smoke visual. |
