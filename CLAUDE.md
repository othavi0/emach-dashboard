...
bun db:sync                        # drizzle-kit push + apply-triggers (rodar após editar schema e após git checkout)
...
# depois: bun db:sync && bun --cwd packages/db db:seed-demo
```
...
| `shared-enums.ts`    | — (só enums)                                                                      | `actor_type` = `pgEnum('actor_type', ['user','system'])`, compartilhado pelas tabelas de auditoria/movimento.                                                                                                                                                                                                          |
...
**Sim:** Cormorant Garamond serif (weight **500**, tracking-tight) em **h1 + h2 de todas as páginas** (display text-5xl / h1 text-4xl / h2 text-2xl), Inter sans no resto (body, h3, controls, sidebar, tabelas), focus = `ring-1 ring-ring ring-offset-2 ring-offset-transparent` (hairline single line com halo herdando surface de baixo, sem border flip), depth via surface contrast (não shadow), `bg-surface-deep` em code/log/featured cards, section bands (`bg-muted/50` border-y) em data zones, `<Image>` do Next, `prefers-reduced-motion: reduce` respeitado.
