...
| T-119 | R6 (AC5-9) | History section + null branch labels | DONE | (pending) | `apps/web/src/app/dashboard/(inventory)/tools/[id]/stock/page.tsx` (secao Historico de movimentacoes, LEFT JOIN via getStockMovements, "Filial removida" quando branchName null, delta colorido verde/vermelho/cinza, empty state pt-BR) |
...
| T-124 | R8 AC14 + R13 | Validation gate + 2 manual checks | DONE | (pending) | Automated: `bun x ultracite check` 142 files clean, `bun --filter=web run build` exit 0 com 13 rotas. Manual: (1) R8 AC14 concurrency test PASS — 2 admin sessions simultaneous adjust produziram movimentos encadeados corretos. (2) R13 AC3 E2E smoke PASS — bun dev fluxo create branch → adjust stock → history row com delta/reason/actor visivel. Validado pelo usuario 2026-04-15. |
