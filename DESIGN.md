...
|---|---|---|---|
| **Stat-card** | Entidade com métricas-resumo (filiais, pedido na listagem) | Header (avatar quadrado de iniciais + nome/nº + meta + status badge) + footer de N métricas | `branches/_components/branch-card.tsx`; `orders/_components/order-card.tsx` (variação **com bloco de itens**, redesign 2026-07-10: sem avatar; header nº mono + cliente + 📍 filial à esquerda, badge de status CAPS + chip mono de transportadora à direita; miolo com até 3 itens `thumb 30px + nome truncado + ×qtd mono` e "+N itens"; footer Unidades (soma) · Total · idade contextual por aba) |
...
- **Pedidos: card com itens + Atrasados + CAPS (2026-07-10)** — redesign da listagem-raiz pra fila de expedição (spec `docs/superpowers/specs/2026-07-10-orders-listing-redesign-design.md`). (1) **Card C1**: itens sempre visíveis (foto+nome+×qtd, 3 máx + "+N"), rodapé **Unidades** (SUM de quantity — "Itens" contava linhas e enganava), idade contextual por aba ("Pago há"/"Enviado há"/"Entregue em"), transportadora em chip mono. (2) **CAPS em badge de STATUS**: `STATUS_BADGE_CAPS` (`components/status-visual.tsx`) aplicado em `OrderStatusBadge` + badges de fulfillment em todas as superfícies — **nunca no `Badge` base** (uppercasearia "Inativa" etc.). (3) **Tab computada Atrasados** (`?tab=late`): `paid`/`preparing` com ≥72h desde `paid_at` saem das tabs de status e existem só em Atrasados (exclusivo, decisão do user); ≥48h = âmbar na fila normal; FIFO por `COALESCE(paid_at, created_at)` nas 3 tabs de expedição (cursor `paidAtAsc`); toast warning ao acessar Pedidos/Separação com atrasados >0. (4) Barra: Período único (`DateRangePicker` com presets), filtro de produto (item aceso no card + resumo de unidades) e de transportadora (facet de `shipping_method`); "Frete a revisar" morreu como filtro (badge no card fica). Regra de tabs split atualizada: a página abre na fila default "Pago" (spec 2026-07-08), não em "todos".
